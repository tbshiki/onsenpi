...
class TestOnsenpiSPAPIClient:
    """OnsenpiSPAPIClientクラスのテストスイート"""
    ...
    def test_client_getattr_routing(self, client):
    ...
    def test_client_getattr_error(self, client):
        """存在しない属性へのアクセスでAttributeErrorが発生することをテスト"""
        with pytest.raises(AttributeError):
            client.non_existent_method()

    def test_client_set_log_level(self, client):
        """ロギングレベル変更機能のテスト"""
        # 初期状態を確認
...
class TestInventoryModule:
    ...
    def test_request_listing_report(self, client, mock_sp_api):
        """inventoryモジュールのrequest_listing_reportメソッドのテスト"""
        # モックの設定
        mock_instance = mock_sp_api["reports"].return_value
        mock_response = mock.MagicMock()
    ...
    def test_report_workflow(self, client, mock_sp_api):
        """inventoryモジュールのレポートワークフローテスト"""
        # request_listing_reportのモック
        mock_reports_instance = mock_sp_api["reports"].return_value
...
class TestProductModule:
    """productモジュールのテストスイート"""

    def test_get_competitive_pricing(self, client, mock_sp_api):
        """productモジュールのget_competitive_pricingメソッドのテスト"""
        # モックの設定
        mock_instance = mock_sp_api["products"].return_value
        mock_response = mock.MagicMock()
...
class TestOrdersModule:
    ...
    def test_get_orders(self, client, mock_sp_api):
        """ordersモジュールのget_ordersメソッドのテスト"""
        # モックの設定
        mock_instance = mock_sp_api["orders"].return_value
        mock_response = mock.MagicMock()
    ...
    def test_get_orders_error(self, client, mock_sp_api):
